...
export function SchoolRegisterModal({ open, onClose }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    setErr("");
    if (!f.school_name || !f.city || !f.coordinator_name || !f.coordinator_email) {
      setErr("Please fill all fields");
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.coordinator_email)) {
      setErr("Enter a valid coordinator email");
      return;
    }
    try {
      setState("loading");
      await axios.post(`${API}/schools/register`, f);
      setState("success");
    } catch {
      setState("idle");
            ...
            {state !== "success" ? (
              <>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="text-[11px] uppercase tracking-[0.28em] text-[#EC5A99] font-bold">
                    School registration
                  </div>
                </div>
                ...
                <p className="mt-2 text-[14px] text-[#4A4A4A] font-body">
                  A coordinator from Titli will reach you within one working day.
                </p>

                <form onSubmit={submit} className="mt-7 space-y-4">
                  <Field label="School name" testid="school-name">
                  ...
                  <Field label="Coordinator name" testid="school-coord">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Field label="Coordinator email" testid="school-email">
                      <input type="email" value={f.coordinator_email} onChange={set("coordinator_email")} data-testid="school-email-input" className={inputCls} placeholder="coordinator@school.edu"/>
                    </Field>
                    <Field label="Phone (optional)" testid="school-phone">
                  ...
                  <div className="text-[11px] uppercase tracking-[0.22em] text-black/45 font-semibold text-center pt-2">
                    Free · No obligation · 80G eligible
                  </div>
                </form>
              </>
            ) : (
              <div className="py-8 text-center">
                ...
                <p className="mt-3 text-[15px] text-[#4A4A4A] max-w-[380px] mx-auto font-body">
                  A coordinator will reach out to <span className="text-[#EC5A99] font-semibold">{f.coordinator_email}</span> within one working day.
                </p>
                <TitliButton onClick={onClose} className="mt-8">Close</TitliButton>
              </div>
            )}
          </motion.div>
        </motion.div>
